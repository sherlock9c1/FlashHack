...
processor_dualp::processor_dualp() {
...
bool processor_dualp::set_x_if_y(int device_id, const region &r, unsigned char x, unsigned char y) {
...
binfile *processor_dualp::bin_for_device(int device_id) {
  Q_UNUSED(device_id);
  return nullptr;
}

bool processor_dualp::flash_write_device(int device_id) {
...
bool processor_dualp::write_region(int device_id, const region &r) {
...
bool processor_dualp::flash_read_device(int device_id) {
...
bool processor_dualp::get_chip_id(int device_id) {
  Q_UNUSED(device_id);
  return true;
}

bool processor_dualp::erase_eeprom(int device_id) {
  ...
  statusmsg("Erasing EEPROM on " + device_name(device_id));
  ...
  forever {
    ...
    // lots of attempts later and still no success.
      ...
      errmsg("Hardware error: Cannot erase EEPROM segment!  This is bad news.");
      return false;
      ...
      emit region_changed(device_id,0x2000,0xDFFF,REGION_ERROR);
      errmsg("Unexpected response in EEPROM erase.");
    }
...
bool processor_dualp::read_device_m2(int device_id, unsigned int read_size, QString file_out) {
  QByteArray out;

  start("Mode 2 device read");

  statusmsg("Starting 64kb memory map dump for device " + device_name(device_id));
...
bool processor_dualp::read_region(int device_id, const region &r) {
  binfile *b = bin_for_device(device_id);
...
bool processor_dualp::_flash_write_device(int device_id) {
   emit progress_changed(0); // progress bar restart

   if(erase_eeprom(device_id) == false) return false;

   if(exec_write_program(device_id) == false) return false;

   binfile *b = bin_for_device(device_id);

   statusmsg("Writing EEPROM on " + device_name(device_id));
...
bool processor_dualp::_unload_kernel(int device_id) {
  if(device_id != TSIDE && device_id != ESIDE) return false;
...
bool processor_dualp::is_booted(int device_id) {
  datastream_request r(device_id,0x0D);
  datastream_reply repl = interface->request(r);
  return repl.success;
}

quint16 processor_dualp::get_checksum(int device_id, quint16 start_address, quint16 end_address) {
  Q_UNUSED(device_id);
...
QString processor_dualp::device_name(int device) {
...
bool processor_dualp::apply_vpp() {
...
bool processor_dualp::remove_vpp() {
...
bool processor_dualp::set_vin(QByteArray vin) {
...
bool processor_dualp::set_calid(QVariant calid) {
  if(is_kernel_loaded(TSIDE) == true) {
...
bool processor_dualp::exec_write_program(int device_id) {
...
bool processor_dualp::exec_read_program(int device_id) {
...
void processor_dualp::update_block_progress(int map_size, int blocks_remaining) {
...
bool processor_dualp::is_kernel_loaded(int device_id) {
  return kernel_version.value(device_id,0) > 0;
}

bool processor_dualp::_load_kernel(unsigned char device_id) {
...
bool processor_dualp::upload_subroutines(unsigned char device_id) {
...
datastream_reply processor_dualp::jump_to_subroutine(int device_id, const program_definition &def) {
...
bool processor_dualp::execute_simple_program(int device_id, QString program_name) {
...
datastream_reply processor_dualp::execute_program(int device_id, QString program_name) {
...
datastream_reply processor_dualp::_execute_program(unsigned char device, const program_definition &def) {
...
datastream_reply processor_dualp::upload_and_execute(unsigned char device, quint16 offset, QByteArray code, int timeout) {
...
bool processor_dualp::enter_programming_mode(unsigned char device) {
...
bool processor_dualp::unlock_security(unsigned char device) {
...
bool processor_dualp::_unlock_security(unsigned char device) {
  // 00 TIMER ACTIVE
  // AA SUCCESS
  // CC KEY INCORRECT
